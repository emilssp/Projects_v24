...
# ╔═╡ d1d59b44-6bc2-423e-ac78-54ae4c874254
using DifferentialEquations, DynamicalSystems, ChaosTools, FindPeaks1D

# ╔═╡ a68eefb7-4f46-41a5-b527-11b069363f80
...
function RosslerEqns!(du,u,p,t)
	...
	du[1] = -y - z 				#x'(x,y,z)
	du[2] = x + a * y  			#y'(x,y,z)
	du[3] = b + z * (x - c) 	#z'(x,y,z)
end

# ╔═╡ ec6bd5a9-01a0-41a6-97e8-1e95d2496a46
function solveRossler(init,tspan, a, b, c; reltol = 1e-9, saveat = 0.01)
	p = [a, b, c]
	prob = ODEProblem(RosslerEqns!, init, tspan, p, reltol = reltol)
	return solve(prob, Tsit5(), maxiters = 1e7, saveat = saveat)
end

# ╔═╡ 6aa72381-faf6-4741-bd3a-efeb6c35d4ad
function plotRossler(tstop= 50.0)
	...
	plt = plot(xlabel = "x", ylabel = "y", zlabel = "z", legendfontsize = 10)
	scatter!(plt, [init[1]],[init[2]],[init[3]], color=:blue, label="Initial point")
	plot!(plt, sol, idxs = (1,2,3), label = "Trajectory", color=:blue, xlabel = "\$x\$", ylabel = "\$y\$", zlabel = "\$z\$", labelfontsize = 20)
	return sol, plt
end
...
function linearModel(x, p) 
	return p[1] .* x .+ p[2]
end

# ╔═╡ c65165fa-827f-467b-b71e-1f91f70c6dd3
function findSlope(t::Vector{Float64},distances::Vector{Float64})
	p0 = [-10.0, 1.0]
	
	fit = curve_fit(linearModel, t, log.(distances), p0)
	slope = fit.param[1]
...
# ╔═╡ ccf63a66-ec01-471f-85e0-e4dad468a5d0
begin
	plt = plotRossler(300)[2] 
	savefig(plt, "Rossler/rossler.png")
end
...
begin
	...
	sol = solveRossler(init,tspan, a, b, c; reltol = 1e-9)
	zn = sol[3,:]
...
begin
	plt2 = plot(xlabel = "\$z_n\$", ylabel = "\$z_{n+1}\$", labelfontsize = 20)
	
	scatter!(plt2, zn[peaks[1:end-1]], zn[peaks[2:end]], msw = 0, alpha = 0.5, markersize = 1.2, color=:blue, label = "")
	plot!(plt2, zn[peaks],zn[peaks], color=:red, label = "", lw= 2)
	r = zn[peaks[1:end-1]]
	m = zn[peaks[2:end]]
	
	
	for i ∈ 50:70
		plot!(plt2, [m[i-1],r[i-1]],[r[i],r[i]], label = "", color=:green)
		plot!(plt2, [r[i],r[i]],[m[i],r[i]], label = "", color=:green)
	end
	plt2 
end

# ╔═╡ d28a33c9-8840-4358-bb65-ddf748bbcaf6
savefig(plt2, "Rossler/map.png")
...
begin
	...
	x0 = solveRossler(init, (0.0, 50), a, b, c; reltol = 1e-9, saveat = 1e-4)[end]
	x1 = x0 + generateRandomVector(NORM)
	sol1 = solveRossler(x0, (0.0, 500), a, b, c; reltol = 1e-9, saveat = 1e-4)
	sol2 = solveRossler(x1, (0.0, 500), a, b, c; reltol = 1e-9, saveat = 1e-4)
	figure = plot()
	scatter!(figure, [x0[1]],[x0[2]],[x0[3]], color=:black)
	scatter!(figure, [x1[1]],[x1[2]],[x1[3]], color=:black)

	plot!(figure, sol1[1,:], sol1[2,:], sol1[3,:], color =:red)
	plot!(figure, sol2[1,:], sol2[2,:], sol2[3,:], color =:blue)
...
function RosslerRandom(cutoff_time)
	init1 = generateRandomVector(NORM)
	x0 = solveRossler(init1, (0.0, 50), a, b, c; reltol = 1e-9, saveat = 1e-4)[end]
	x1 = x0 + generateRandomVector(NORM)
	sol1 = solveRossler(x0, (0.0, cutoff_time), a, b, c; reltol = 1e-9, saveat = 1e-4)
	sol2 = solveRossler(x1, (0.0, cutoff_time), a, b, c; reltol = 1e-9, saveat = 1e-4)
...
begin
	...
	logplot1 = plot(ylabel = "\n\$\\log||\\textbf{x}_0 - \\textbf{x}_1||\$", xlabel = "\$ t \$", labelfontsize = 20)
	tt = collect(0.0:diff(sol1.t)[1]:sol1.t[cutoff])
	ic, slope = findSlope(tt,distance1[1:cutoff])
	plot!(logplot1, sol1.t, log.(distance1),color=:red, label ="", lw = 1.5)
	plot!(logplot1, tt, ic .+ slope .* tt,color=:blue, label = "", lw = 1.5)

	# scatter!(logplot1, [sol1.t[cutoff]],[log.(distance1[cutoff])])
	logplot1
end

# ╔═╡ 7885c99c-ba29-433a-a29f-1c8923362990
savefig(logplot1, "Rossler/logplot.png")
...
begin
	...
	for _ ∈ 1:10
		times, distances = RosslerRandom(cutoff_time)
...
begin
	...
	plot(xlabel="\$t\$\n", ylabel = "\n\$||\\textbf{x}_0 - \\textbf{x}_1||\$", labelfontsize = 20, legendfontsize = 14, size = (800,300))
	plot!(t, distance1[1:cutoff], color=:red, label = "Simulation", lw=1.5)
	plot!(t, δ.(t), color=:blue, label = L"\delta(t) = \delta_0 e^{\lambda t}", lw = 1.5)
end

# ╔═╡ 119d7d17-d447-45b8-820d-377a8beee825
savefig("Rossler/r_separation.png") 
