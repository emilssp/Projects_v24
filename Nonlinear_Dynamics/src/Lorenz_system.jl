...
using DifferentialEquations, DynamicalSystems, ChaosTools,PlutoUI

# ╔═╡ c52b14ab-a89d-4ac1-9f3e-9fdce3f144a7
using GLMakie

# ╔═╡ dc412de9-83f1-4da0-bc1b-c67930e18156
using DataStructures: CircularBuffer

# ╔═╡ 58248e94-22a1-4d63-9066-d56c3f867088
using FindPeaks1D

# ╔═╡ bba63a56-3aca-4607-baeb-bb66123dc668
using Random, LinearAlgebra

# ╔═╡ 8f82afe3-4abe-4592-95fb-ca6d90b3add8
using LsqFit, Statistics

# ╔═╡ 8057909a-5a03-4dd7-890d-7abbe72832db
function LorenzEqns!(du,u,p,t)
	σ, ρ, β = p
	x, y, z = u
	du[1] = σ * (y - x) 	#x'(x,y,z)
	du[2] = x * (ρ - z) - y #y'(x,y,z)
	du[3] = x * y - β * z 	#z'(x,y,z)
end

# ╔═╡ bb313d25-97b7-4448-ac06-642131c98eed
function LorenzStep(u0, σ, ρ, β)
	p = [σ, ρ, β]
	tspan =(0.0,1.)
	prob = ODEProblem(LorenzEqns!, u0, tspan, p, abstol =1e-9, reltol = 1e-9)
	sol = init(prob, Tsit5())#, adaptive = false, dt = 0.0001) 
	step!(sol)
	return sol
end

# ╔═╡ 84ae9ee1-095a-4f57-8306-a4121e2a809a
function solveLorenz(u0,tspan, σ, ρ, β; reltol = 1e-9, saveat = 0.01)
	p = [σ, ρ, β]
	prob = ODEProblem(LorenzEqns!, u0, tspan, p, reltol = reltol)
	return solve(prob, Tsit5(), maxiters = 1e7, saveat = saveat)
end

# ╔═╡ 7c1e2c99-6a38-41e9-a0dd-c1b674a3ec06
function animLorenz!(pos,traj, σ, ρ, β, ax)
			u = [pos[][1], pos[][2], pos[][3]]
			x,y,z = LorenzStep(u, σ, ρ, β).u
			pos[] = Point3([x,y,z])
			push!(traj[],Point3f(x,y,z)) #does not use assignment
			traj[]=traj[] #trigger the update on the plot	
end

# ╔═╡ 25ef0311-fa29-4374-9d6b-f7f43d3e4588


# ╔═╡ 482743ad-3c40-44a1-ac6b-dd62f90d08ab
function makefigLorenz(u0, σ, ρ, β)
	pos = Observable(Point3(u0[1],u0[2],u0[3]))
	traj = CircularBuffer{Point3}(10000)
	fill!(traj,Point3(u0[1],u0[2],u0[3]))
	traj = Observable(traj)
	fig = Figure(); display(fig)
	ax = Axis3(fig[1,1])
	
	GLMakie.scatter!(ax,pos,marker=:circle)
	GLMakie.lines!(ax, traj)
	GLMakie.xlims!(ax, -20,20)
	GLMakie.ylims!(ax, -20,20)
	GLMakie.zlims!(ax, -20,50)

	return fig, ax, pos, traj
end

# ╔═╡ 3a56d2de-4494-4663-bfea-ba506895ffb6
function runLorenzSimulation()
	σ=10.
	ρ=28.
	β=8/3
	
	u0=[0.001,0.001,0.001]
	fig, ax, pos, traj = makefigLorenz(u0, σ, ρ, β )
	run = GLMakie.Button(fig[2,1]; label = "Run", tellwidth = false)
	isrunning = Observable(false)
	on(run.clicks) do clicks; isrunning[] = !isrunning[];end
	on(run.clicks) do clicks
		@async while isrunning[]
			isopen(fig.scene) || break
			animLorenz!(pos,traj, σ, ρ, β, ax)
			sleep(0.00001)
		end
	end
end

# ╔═╡ f7e887de-2992-4b0e-9ac5-14530e022eae
function plotLorenz(tstop= 50.0)
	σ=10.
	ρ=28.
	β=8/3
	...
	u0=[0.001,0.001,0.001]
	tspan3 = (0, tstop)
	sol = solveLorenz(u0,tspan3, σ, ρ, β, reltol = 1e-9, saveat = 0.001)
	fig = Figure()
	ax = Axis(fig[1, 1], xlabel = L"$x$", ylabel =L"$z$", xlabelsize=30, ylabelsize=30)
	ax.xgridvisible[] = false
	ax.ygridvisible[] = false
	scatter!(ax, Point3(u0), color=:red)
	plot!(ax, sol, idxs = (1,3))
	return sol, fig
end

# ╔═╡ 4313e538-1380-4500-9bec-980fe53607b4
begin
	sol80, fig = plotLorenz(80.0)
	fig
end 

# ╔═╡ 3fdceb1d-5697-4ca1-a5b8-f8119521efbb
begin
	fig0 = Figure(size = (800,300))
	ax0 = Axis(fig0[1, 1], ylabel = L"$z$", xlabel =L"Time $t$", xlabelsize=24, ylabelsize=30)
	plot!(ax0, sol80, idxs = (3))
	fig0
	save("Lorenz/z_plot.png", fig0)
end

# ╔═╡ 7964b04a-0efd-4c8b-b5ff-cb4702e880a5
save("Lorenz/Lorenz.png", fig)

# ╔═╡ 646f2eb3-094b-44cc-ae58-8871e96c15b3
begin
	σ=10.
	ρ=28.
	β=8/3
	...
	u0=[0.001,0.001,0.001]
	tspan3 = (0, 1e5)
	sol = solveLorenz(u0,tspan3, σ, ρ, β; reltol = 1e-9)
	zn = sol[3,:]
	peaks = findpeaks1d(zn)[1]
end

# ╔═╡ 3f8f81e4-d2f0-482b-b2f8-d6506b870120
begin
	fig2 = Figure()
	ax2 = Axis(fig2[1, 1], xlabel = L"\text{z}_n", ylabel =L"\text{z}_{n+1}", xlabelsize=30, ylabelsize=30)

	r = zn[peaks[1:end-1]]
	m = zn[peaks[2:end]]
	
	scatter!(ax2, r, m, markersize = 3, alpha=0.5)
	lines!(ax2, r,r, color=:red)
	
	for i ∈ 50:90
		lines!(ax2, [m[i-1],r[i-1]],[r[i],r[i]], label = "", color=:green)
		lines!(ax2, [r[i],r[i]],[m[i],r[i]], label = "", color=:green)
	end
	fig2 
end

# ╔═╡ 2e0f67d5-0c14-43b3-b36d-05caf1cd6d01
save("Lorenz/Lorenz_map.png", fig2) 

# ╔═╡ a5d8f35c-8b32-4e43-b989-cc8ced1beae4
...
# ╔═╡ 6e4f47b6-e0f9-4c6a-912e-eacec115901f
Random.seed!(123)

# ╔═╡ 571f930c-ef8a-48e2-8925-b0136d7838df
function generateRandomVector(norm_)
    point = randn(3)
    return (norm_ / norm(point)) .* point
end

# ╔═╡ f0b79b72-d8dd-4dec-bf5d-ecf5b49e457c
begin
	const NORM = 1e-6
	init1 = generateRandomVector(NORM)
end

# ╔═╡ ee97cfc7-64b3-40d8-a6a6-53a3786297fa
function LorenzRandom(cutoff_time)
	init1 = generateRandomVector(NORM)
	x0 = solveLorenz(init1, (0.0, 50), σ, ρ, β; reltol = 1e-9, saveat = 1e-4)[end]
	x1 = x0 + generateRandomVector(NORM)
	sol1 = solveLorenz(x0, (0.0, cutoff_time), σ, ρ, β; reltol = 1e-9, saveat = 1e-4)
	sol2 = solveLorenz(x1, (0.0, cutoff_time), σ, ρ, β; reltol = 1e-9, saveat = 1e-4)
...
begin
	...
	figure = Figure()
	axis = Axis(figure[1, 1], xlabel = L"$x$", ylabel =L"$z$", xlabelsize=30, ylabelsize=30)

	lines!(axis, Point2.(sol1[1,:],sol1[3,:]), color =:red)
	lines!(axis, Point2.(sol2[1,:],sol2[3,:]), color =:blue)
	scatter!(axis, Point2(x0[1],x0[3]), color=:black)
	scatter!(axis, Point2(x1[1],x1[3]), color=:black)
	save("Lorenz/two_traj.png", figure)
	figure
end

# ╔═╡ 285ad64a-b2c5-4e9d-9d22-d307a20317a7


# ╔═╡ 57c8cf52-9967-4fcf-a10f-39db074c819a
begin
	distance1 = [norm(vec) for vec ∈ (sol1 .- sol2)]
end

# ╔═╡ 69791fd9-9986-41f1-8c53-b13b76665b6c
model(x, p) = p[1] .* x .+ p[2]

# ╔═╡ c6a66e7b-b9a6-4689-9b3f-d8d7efd9da01
function findSlope(t::Vector{Float64},distances::Vector{Float64})
	p0 = [-10.0, 1.0]
	
	fit = curve_fit(model, t, log.(distances), p0)
	slope = fit.param[1]
...
begin
	logplot1 = Figure()
	logax1 = Axis(logplot1[1, 1], xlabel = "Time", ylabel =L"$\log||\textbf{x}_0 - \textbf{x}_1||$", xlabelsize=22, ylabelsize=22)
	intercept, slope = findSlope(sol1.t[1:200000], distance1[1:200000])
	lines!(logax1, sol1.t[1:200000], model(sol1.t[1:200000], [slope,intercept]))
	lines!(logax1, sol1.t, log.(distance1),color=:red)
	# scatter!(logax1, sol1.t[200000],log(distance1[200000]), color=:black, markersize = 10)
	cutoff_time = sol1.t[200000]
	logplot1
end

# ╔═╡ 5806e304-b07a-4dfd-babb-a68baba6c433
save("Lorenz/logplot1.png", logplot1)

# ╔═╡ 95e3a7df-e6bb-40e2-bc6c-07b339bd0494
begin
	λ = Float64[]
	final_logdistance = Float64[]
	for _ ∈ 1:100
		times, distances = LorenzRandom(cutoff_time)
		intercept_temp, λ_temp = findSlope(times,distances)
		push!(λ,λ_temp)
		push!(final_logdistance, log(distances[end]))
	end
	λ
end
...
begin
	figure2 = Figure(dpi =200, legendfontsize = 12, size = (800,300))
	axis2 = Axis(figure2[1, 1], xlabel = L"Time $t$", ylabel =L"$||\textbf{x}_0 - \textbf{x}_1||$", xlabelsize=22, ylabelsize=22)
	dt_steps = diff(sol1.t)[1]
	t = collect(0.0:dt_steps:sol1.t[200000])
	lines!(axis2, t, δ.(t), color=:blue, label = L"$\delta(t) = \delta_0 e^{\lambda t}$")
	lines!(axis2, t, distance1[1:200000], color=:red, label = "Simulation")

    axislegend(axis2, position = :lt, labelsize=22)
end

# ╔═╡ a5d4281e-903f-4e2d-ad3a-ccfa5592d7ee
begin
	save("Lorenz/separation.png", figure2, dpi=50) 
	figure2
end 

# ╔═╡ e35e4b96-e5e3-4969-979a-37b2e84a4890
...
manifest_format = "2.0"
project_hash = "fb12799c98ccfbb9ac3a000b05b0f036d329a536"
...
git-tree-sha1 = "895833094aa539c856ba686381909d7c7e535b3e"
...
deps = ["Combinatorics", "DelayEmbeddings", "Distances", "FFTW", "LinearAlgebra", "Neighborhood", "QuadGK", "Random", "Reexport", "SparseArrays", "SpecialFunctions", "StateSpaceSets", "StaticArrays", "Statistics", "StatsBase", "Wavelets"]
git-tree-sha1 = "bf6898c5ef0f416a90664c997f0de46b0c5dcb7f"
...
git-tree-sha1 = "5a8c0a7c2a4fe94a5b72195f7c38159cdf1f47b9"
...
git-tree-sha1 = "a20272be82e0f712651c5a76f0df985f5fd5d6fe"
