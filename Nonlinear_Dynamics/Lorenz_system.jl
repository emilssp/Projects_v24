...
begin
	fig2 = Figure()
	ax2 = Axis(fig2[1, 1], xlabel = L"\text{z}_n", ylabel =L"\text{z}_{n+1}")
	scatter!(ax2, zn[peaks[1:end-1]], zn[peaks[2:end]], markersize = 0.2)
	lines!(ax2, zn[peaks],zn[peaks], color=:red)
	fig2 
end
...
begin
	...
	figure = Figure()
	axis = Axis(figure[1, 1], xlabel = "x", ylabel ="z")
...
begin
	logplot1 = Figure()
	logax1 = Axis(logplot1[1, 1], xlabel = "Time", ylabel =L"\log[\text{Distance}]")
